...
export async function GET(req, { params }) {
  try {
    await connectDB();
    const cert = await Certs.findById(params.id);
    if (!cert) {
    ...
    return NextResponse.json(
      { error: "Failed to fetch cert", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function PUT(req, { params }) {
  try {
    ...
    const cert = await Certs.findByIdAndUpdate(params.id, updates, {
      new: true,
    });
    if (!cert) {
      return NextResponse.json({ error: "Cert not found" }, { status: 404 });
...
export async function DELETE(req, { params }) {
  try {
    await connectDB();
    const cert = await Certs.findByIdAndDelete(params.id);
    if (!cert) {
      return NextResponse.json({ error: "Cert not found" }, { status: 404 });
    }
    return NextResponse.json(
